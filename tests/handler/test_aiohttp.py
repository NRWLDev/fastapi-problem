...
class TestExceptionHandler:
    ...
    async def test_unexpected_error(self, monkeypatch):
        ...
        assert response.status == constant.SERVER_ERROR
    ...
    async def test_aiohttp_error(self):
        ...
        assert response.status == constant.NOT_FOUND
        assert json.loads(response.text) == {
            "message": "HTTPNotFound",
            "debug_message": "404: Not Found",
            "code": None,
        }
    ...
    async def test_known_error(self):
        ...
        assert response.status == constant.BAD_REQUEST
        assert json.loads(response.text) == {
            "message": "This is an error.",
